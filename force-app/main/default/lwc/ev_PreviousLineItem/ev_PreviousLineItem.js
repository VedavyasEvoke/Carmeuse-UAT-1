...
import getPreviousQuoteLineDetails from '@salesforce/apex/ev_QuoteInfo.retrivePreviousQuoteLine';

export default class ev_PreviousLineInfo extends LightningElement {
    @api recordId;
    ...
    @wire(getPreviousQuoteLineDetails, { recordIdAccount: '$recordId' })
            ...
            this.columns.forEach(column => {
                if (column.fieldName === 'Surcharge__c') {
                 ...
                 if (row.SBQQ__Quote__r?.Variable_Fuel_Surcharge__c) {
                    console.log('In True', row.Surcharge_variable__c);
                    record[column.fieldName] = row.Surcharge_variable__c ?? 0;
                } else {
                    console.log('In False', row.Surcharge__c);
