...
import { categoryOptions } from './ev_PricingCondition_NorthAmerica_Service';
...
*/
export default class Ev_PricingCondition_NorthAmerica extends NavigationMixin(LightningElement) {
    @api recordId;
    ...
    decimalPlaces = 2;
    //table columns
    qliColumns = [{ label: 'Estimated Qty/ Delivery', fieldName: 'Estimated_Qty_Delivery__c', type: "Decimal", editable: false, columnLevelHelp: 'Only editable for Flat Category Options' },
    ...
    //condition based table columns
    qliColumnsRefs = {
        List_Unit_Price__c: { label: 'Unit Price', fieldName: 'List_Unit_Price__c', type: "Decimal", editable: true, required: true, minValue: '0.01' },
    ...
    getQuotePricingConditionFieldset() {
        fetchQuotePricingConditionFieldset({fieldSetName: 'Pricing_Condition_Header_Fields'}).then(result => {
            if (result) {
    ...
    getQliPricingConditionFieldset() {
        fetchQliPricingConditionFieldset({fieldSetName: 'Pricing_Condition_Header_Fields'}).then(result => {
            if (result) {
    ...
    preparePricingCategsForQLIs(pricingCategs, pcas) {
        ...
        if (pricingCategsMap && this.qlis) {
            this.qlis.forEach(qli => {
                ...
                if (qli.attributes.Product && !qli.attributes.Product.value && qli.attributes.Product.options && qli.attributes.Product.options.length > 0) {
        ...
        try {
            ...
            qli.attributes[category]?.data.forEach(row => {
                if (row.currentOption?.value?.length > 0 && !isNaN(row.price)) {
                   ...
                   /* if (row.currentOption.label.toLowerCase().includes('flat')&& row.price > 0 ){
                        qli.MIN_LOAD_SIZE__c = 27;
                        if(typeof this.template.querySelector(`[data-id="${qli.Id}"][data-fieldname="MIN_LOAD_SIZE__c"]`) != 'undefined')
                         this.template.querySelector(`[data-id="${qli.Id}"][data-fieldname="MIN_LOAD_SIZE__c"]`).value=27;
                    }
                    */
    ...
    handleRowCategChange(event) {
        try {
            ...
            if (value) {
                //this.qlis[index][value] = isNaN(previousData.price) ? 0 : Number(previousData.price);
                let defaultPrice = this.assignDefaultPrice(this.qlis[index], value);
                ...
                this.qlis[index].attributes[category].data[rno].price = 0; //Making the Price to 0 if "None" option is selected
    ...
    assignDefaultPrice(qli, categoryValue) {
        ...
        let priceList = this.priceLists?.filter(priceList => {
            return priceList.CnTy__c === categoryValue && priceList.Plnt__c === qli.Plant__c && priceList.Material__c === qli.SBQQ__ProductCode__c;   //return priceList.CnTy__c === categoryValue;
        });
    ...
    assignDefaultPrices(qli) {
        try {
            let categories = [{ name: 'Product', isMultiSelect: false }, { name: 'Commission', isMultiSelect: false }, { name: 'Discount', isMultiSelect: false }, { name: 'Freight', isMultiSelect: true }, { name: 'Surcharges', isMultiSelect: true }, { name: 'Fees', isMultiSelect: true }];
            categories.forEach(category => {
                ...
                } else if (!category.isMultiSelect && qli.attributes[category.name]?.value && !(qli.attributes[category.name]?.price && Number(qli.attributes[category.name].price) !== 0)) {
                    qli.attributes[category.name].price = this.assignDefaultPrice(qli, qli.attributes[category.name]?.value);
                }
            });
        } catch (e) {
            console.log(e);
    ...
    handleClose() {
        this.navigateToUrlFromVf('/' + this.recordId);
    }
    ...
    saveQLIs(closePage) {
        ...
        if (isValid === true) {
            ...
            updateQLIsWithPricingCategs({ qlis: qlisData }).then(result => {
                if (result && closePage) {
                    this.handleBack();
                }
                this.spinner = false;
    ...
    handleQuoteSubmit() {
        this.template.querySelectorAll('[data-objectname="SBQQ__Quote__c"]').forEach(form => form.submit());
    ...
    isValid() {
        ...
        let stivs = this.checkForValidIncoterms();

        let status = cfvo && cfvdid && cffs && stivs;
